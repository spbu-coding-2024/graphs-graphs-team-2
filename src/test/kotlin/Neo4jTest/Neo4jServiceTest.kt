...
@SpringBootApplication @ComponentScan(basePackages = ["io.ioNeo4j", "Neo4jTest"]) class TestApp
...
class Neo4jServiceTest {
    ...
    fun `test write and read random graph`(correctGraph: Graph) {
        ...
        for (i in 0..<sortedGraphEdge.size) {
            ...
            assertEquals(
                sortedGraphEdge[i].vertices.first.label,
                sortedReadGraphEdge[i].vertices.first.label,
            )
            assertEquals(
                sortedGraphEdge[i].vertices.second.label,
                sortedReadGraphEdge[i].vertices.second.label,
            )
