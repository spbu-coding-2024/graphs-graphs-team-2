...
import inpout.Graphs.graphName
import inpout.Vertices.autoIncrement
import inpout.Vertices.references
import inpout.Vertices.uniqueIndex
import inpout.Vertices.vertex
...
class SQLiteEXP(dbPath: String)  {
    init{
        Database.connect("jdbc:sqlite:$dbPath", driver = "org.sqlite.JDBC", setupConnection = { it.createStatement().execute("PRAGMA foreign_keys=ON") })
        transaction {
    ...
    fun findVertices(graphId :Int):List<vertexInfo>{
        val p= transaction {
            Vertices.select{Vertices.graph_id eq graphId}.map{ vertexInfo(it[Vertices.vertex], it[Vertices.x], it[Vertices.y])}
        }
        return p
    }
